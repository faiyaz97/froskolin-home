...
Both routes orchestrate extraction through server-only `analyzeBill` in `src/lib/bills/analyze-bill.ts`. Initial Gemini calls use temperature 0 and medium thinking to limit latency; targeted repairs retain high thinking. Both passes use structured generation schemas followed by full strict Zod parsing. Temperature 0 reduces sampling variation but does not guarantee identical extracted facts. Completion diagnostics contain only utility type, analysis status, issue count, and whether the two calculated buckets are present; they never contain bill values, labels, evidence, filenames, or account details.
