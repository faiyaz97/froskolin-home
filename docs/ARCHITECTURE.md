...
Group invitation links contain the group code and group PIN in the `/join` URL fragment, which is read and validated only in the browser and is not sent in the initial HTTP request. The link is a bearer invitation, not encryption or a separate expiring token: anyone holding it has the existing group join credentials. Only admins with access to the group PIN can generate it in Settings. The recipient supplies their own name and personal PIN and explicitly submits the existing join action, which rechecks the current code, PIN, and joining-enabled state. Changing the group code or PIN invalidates old links; disabling joining blocks them while disabled. No personal PIN, internal auth alias, or service credential belongs in an invitation URL.

Invitation PNGs are rendered in the browser from same-origin brand artwork and the admin-visible group details. They are not uploaded, persisted, or exposed through public image/OG endpoints. The dialog prepares a file before the share click to retain browser user activation. Where supported, the Web Share API hands the image and invitation text together to a user-selected app, with an optional user-triggered text fallback if the target drops the caption; websites cannot force a specific share target or guarantee that target retains captions. Unsupported or failed sharing offers a downloadable image and explicit social-link actions; cancellation stays silent. Object URLs are revoked when the dialog unmounts. Both the image and link grant knowledge of join credentials and should only be sent to intended recipients.
...
The direct extraction route has temporary development-only structured diagnostics for source-page debugging. They are disabled unless `NODE_ENV` is `development` and report the prepared page count/input mode, schema-validated financial rows, analysis issues, and any targeted repair result. Invoice identity and free-form evidence are omitted, row text is redacted again, and no document bytes, filenames, credentials, or raw provider response are logged. Production retains the allowlisted diagnostics above.

PDF preparation extracts a bounded, redacted text layer or renders metadata-free page images when supported. If the serverless PDF parser cannot decode a provider-specific PDF, extraction falls back to sending the already size/type-validated original PDF to Gemini, which supports PDF input directly. The fallback logs only the preparation stage and file format; it never logs bill content, filenames, or parser errors.
...
Both routes orchestrate extraction through server-only `analyzeBill` in `src/lib/bills/analyze-bill.ts`. Initial and repair Gemini calls use temperature 0, high thinking, and the same structured generation schema, followed by full strict Zod parsing. Temperature 0 reduces sampling variation but does not guarantee identical extracted facts. Completion diagnostics contain only utility type, analysis status, issue count, and whether the two calculated buckets are present; they never contain bill values, labels, evidence, filenames, or account details.
...
After successful financial writes, the server awaits best-effort Web Push delivery before completing the action so serverless runtimes keep the request alive. Delivery failures never change the successful financial result. Only involved active group members receive messages; the actor is excluded. Creation covers expenses, utility bills, settlements, and newly generated recurring occurrences. Expense/bill edits and absence recalculation compare financial shares and notify affected participants (including those removed from a split). Payment edits/voids, settings changes, and mere recurring-rule edits do not currently send push. The lock-screen title uses the expense, bill, or payment title. Its body names the payer, total paid, and the recipient's amount from that recipient's perspective, then opens the corresponding authenticated record. PINs and unrelated group details are never sent in the body.
