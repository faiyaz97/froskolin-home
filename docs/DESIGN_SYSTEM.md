...
- Group Settings provides an admin-only “Invite members” icon in the existing group header row, without an extra row beneath credentials. Its shared dialog previews a locally generated square invitation card with the existing cat artwork, brand, group name, code, and PIN. A selectable link has icon-only Copy and Download actions; a row of WhatsApp, Telegram, Instagram, Facebook, and More icons uses accessible names, title tooltips, and 44px targets. Lucide supplies general controls; small local SVGs supply brand marks absent from the installed icon set. Disable invitations while joining is off or the group PIN is unavailable. Native file sharing sends the prepared PNG and invitation text together in one share request. The text has a title, blank line, Group code and Group pin on separate lines, blank line, then the invitation link; the optional text action uses the same layout if the receiving app drops the caption. Unsupported devices receive image-download and social-link fallback actions. Keep development/localhost explanations out of the product dialog.
...
- Use compact document/attachment, void, and edit icon tools below the card. Only render the left document tool when a file exists; ordinary expenses use a paperclip and utility bills use the bill-document icon. Open private files in the in-app attachment page, with PDF pages or images rendered inside the app and an explicit close action returning to the expense. Keep file access authorized on the server.
